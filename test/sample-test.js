...
describe("NFTFactory", function () {
  ...
  it("after endAuction getAccountAuctionByIndex should return aauction with isEnd==true", async () => {
    ...
    await nftAuction.connect(addr1).startAuction(3, 1, 60);
    await nftAuction.connect(addr1).startAuction(4, 1, 60);
    await nftAuction.endAuction(0);
    await nftAuction.endAuction(1);
    await nftAuction.endAuction(2);
    await nftAuction.endAuction(3);
    const auctionsAll = await nftAuction.getAllAuctions();
    console.log(
  ...
  it.only("should return auction Id using bids", async () => {
    ...
    await nftAuction.connect(addr1).startAuction(3, 1, 60);
    await nftAuction.connect(addr1).startAuction(4, 1, 60);
    await nftAuction.connect(addr1).startAuction(5, 1, 60);
    await nftAuction
      .connect(addr2)
    ...
    await nftAuction
      .connect(addr2)
      .riseBid(2, { value: ethers.utils.parseUnits("10", "wei") });
    await nftAuction
      .connect(addr2)
      .riseBid(3, { value: ethers.utils.parseUnits("10", "wei") });
    await nftAuction
      .connect(addr2)
      .riseBid(5, { value: ethers.utils.parseUnits("10", "wei") });
    await nftAuction
      .connect(addr3)
